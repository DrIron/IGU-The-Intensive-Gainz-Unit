...
- **Add picker is shared**: `SessionAddPicker.tsx` is the single picker used by both `SessionBlock`'s desktop popover (`variant="compact"`) and `MobileDayDetail`'s drawer (`variant="roomy"`). Built-in: search, "Recently used" row (top-5 strength muscles in the current week, computed once in `MuscleBuilderPage`), and `×N` placement-count badges sourced from `useMusclePlanVolume(slots).placementCounts`. Strength sessions get a body-region accordion + subdivisions; non-strength sessions get only their type's activities. **Don't render the body-region or activity list inline anywhere else** — extend the shared picker. The desktop right-rail palette (`MusclePalette` + `DraggableMuscleChip`) and the tablet "Muscles" Sheet were removed when this shipped (Apr 26); the per-session inline `+` is the only add path. The "+" button on each `SessionBlock` is `opacity-50` always (not hover-only) so the picker is reachable on touch devices.
