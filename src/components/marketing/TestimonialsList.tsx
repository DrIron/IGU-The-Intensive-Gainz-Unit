...
interface TestimonialsListProps {
  /** Cap the number of approved testimonials shown (e.g. Index preview). Undefined = all. */
  limit?: number;
  /** Reserved: filter to one coach's testimonials (deferred filter-by-coach feature). */
  coachId?: string;
  className?: string;
}

/**
 * Shared read-only grid of APPROVED testimonials (anon-readable rows). Used by both the Index
 * preview (limit=3) and the standalone /testimonials view — one source, no divergence. Flat PUB8
 * language (bg-card border, fill-primary stars, no shadows/gradients).
 */
export function TestimonialsList({ limit, coachId, className }: TestimonialsListProps) {
  ...
  useEffect(() => {
    const key = `${limit ?? "all"}:${coachId ?? "any"}`;
        ...
        testimonials.map((testimonial) => (
          <div key={testimonial.id} className="bg-card border border-border rounded-lg p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold">{testimonial.author_display_name || "Client"}</p>
                {testimonial.coaches && (
