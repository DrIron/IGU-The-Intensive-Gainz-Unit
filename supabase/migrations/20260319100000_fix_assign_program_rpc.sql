-- ============================================================
-- Fix assign_program_to_client RPC:
-- 1. Copy session_type and session_timing to client_day_modules
-- 2. Include column_config in prescription_snapshot_json
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- 4. Copy PUBLISHED modules only (now includes session_type + session_timing)
    FOR v_module IN
      SELECT *
      FROM day_modules
    ...
    LOOP
      ...
      -- 5. Copy exercises + prescription snapshots (now includes column_config)
      FOR v_exercise IN
        SELECT me.*
