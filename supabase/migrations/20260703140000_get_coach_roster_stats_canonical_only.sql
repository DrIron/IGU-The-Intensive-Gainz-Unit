-- P5 Legacy drop, Stage A: get_coach_roster_stats.has_program is now canonical-ONLY.
-- Coverage is 0 (every active client has an active client_plan_assignment — verified
-- by the pre-Stage-A gate), so the legacy `client_programs` arm of the `prog` UNION can
-- never contribute a row the canonical arm doesn't already cover. Drop it, leaving only
-- the canonical arm. Everything else byte-identical to 20260630104618.
-- CREATE OR REPLACE preserves grants (authenticated EXECUTE; anon none). No signature change.
-- NOTE: keep the `client_program_status` enum — it is REUSED by client_plan_assignment.status.
...
BEGIN
  ...
  -- has_program: active canonical assignment only (P5 Stage A — legacy client_programs
  -- arm dropped; coverage 0 means the canonical arm is complete).
  prog AS (
    SELECT client_id AS user_id
    FROM public.client_plan_assignment
    WHERE client_id IN (SELECT user_id FROM roster)
      AND status = 'active'::client_program_status
  )
