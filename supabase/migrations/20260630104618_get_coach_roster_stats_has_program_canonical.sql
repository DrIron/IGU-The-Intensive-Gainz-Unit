-- P5 (server-side): make get_coach_roster_stats.has_program canonical-aware.
-- Only the `prog` CTE changes: has_program = active client_programs OR active
-- client_plan_assignment. Additive/monotonic (false->true only for canonical-only
-- clients; every legacy client unchanged). Everything else byte-identical.
-- CREATE OR REPLACE preserves existing grants (authenticated EXECUTE; anon none).
...
BEGIN
  ...
  -- has_program: active legacy program OR active canonical assignment (P5 burn-down).
  -- Monotonic add of the canonical arm; the client_programs arm is dropped at the
  -- eventual legacy-table cutover.
  prog AS (
    SELECT user_id
    FROM public.client_programs
    WHERE user_id IN (SELECT user_id FROM roster)
      AND status = 'active'
    UNION
