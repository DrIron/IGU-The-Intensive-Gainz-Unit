...
-- CC cast fix (identical to migration_2): the category column is the exercise_category enum;
-- a text-typed value won't assign into it (SQLSTATE 42804). No data-logic change -- same value,
-- same rows. The dry-run passed because it used a text-category shadow that masked the enum.
