...
> **⚠️ Prerequisite — stable slot identity (gates canonical logging going authoritative).** P1's `save_plan_from_builder` *delete-and-recreates* `plan_*` on every save, so `plan_slots.id` churns and `exercise_set_logs.plan_slot_id` (currently `ON DELETE SET NULL`) orphans the client's logs on every coach edit — acceptable for P3's flagged/dev use, **not** for authoritative canonical logging. Fix before the P3→authoritative cutover: convert the materializer to an **identity-preserving upsert** keyed on the builder's stable slot/session ids (already persisted in `slot_config` as `MuscleSlotData.id` / `SessionData.id`) — store them on `plan_slots`/`plan_sessions` and UPSERT (update unchanged, insert new, delete removed) instead of wiping. Then `plan_slot_id` is stable across saves, template edits still flow live, and `SET NULL` becomes a true "slot deleted" edge case. (Flagged by the terminal in the P3 migration + resolver TODOs, 2026-06-27.)
