...
- **P4 — coach-client editor (B4).** The Planning Board, scoped to an `assignment` → edits the client's own canonical **clone** directly via `save_plan_direct` (board_v2 own-your-copy; the earlier override-writing design is retired). Drag sessions, day-sync, progression copy-paste, per-client deload — all "for free" because it's the same engine. Shipped + de-flagged (gated on `board_v2`).
