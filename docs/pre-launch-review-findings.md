...
### B6-N3 + N5 + N9 + N10 -- shipped 2026-05-31 (FE + cancel-session edge fn; no migration)

- **B6-N3 ✅ FIXED** -- `src/components/client/EnhancedWorkoutLogger.tsx` `completeWorkout()`. The `client_day_modules.update({status:'completed'})` was awaited without `{ error }` or a rows-affected check, then fired the success toast -- an RLS denial (HTTP 200, 0 rows) left the module `scheduled` in the DB while the client saw "completed". Fixed with `.select('id')` + `if (error) throw error;` + `if (!data || data.length === 0) throw new Error("Workout completion not persisted")`. Same shape as PR #117. Verified: the `client_day_modules_update_clients_own` UPDATE policy is `USING (cp.user_id = auth.uid())`, so a wrong-client update 0-rows silently -- now caught.

- **B6-N9 ✅ FIXED** -- `src/hooks/useProgramCalendar.ts` `copyWeek()`. The `exercise_prescriptions.insert(...)` was unchecked while its sibling `module_exercises` insert in the same loop checks `{ error }`. A silent RLS denial left the copied module with no prescription. Fixed: destructure `{ error: prescError }` + throw.

- **B6-N10 ✅ FIXED** -- `src/components/coach/CoachSessions.tsx` `handleAddSlots()` built slot times via `new Date(slotDate).setHours(...)` + `.toISOString()`, anchoring to the browser's local TZ -- a coach on a non-Kuwait device created slots offset by hours. New `src/lib/kuwaitTime.ts` `kuwaitWallClockToUtc({year,month,day,hour,minute})` anchors to Asia/Kuwait (UTC+3, no DST), mirroring the server `get_current_week_bounds()` RPC. Verified TZ-independent: 09:00 Kuwait Tue → `06:00:00Z` identically under TZ=America/Los_Angeles, Asia/Tokyo, Pacific/Kiritimati (UTC+14), UTC.

- **B6-N5 ✅ FIXED (re-scoped)** -- The audit specced a new `refund_session` RPC reversing weekly credit + coach earnings. **That premise was wrong against the actual post-B6-N1 architecture:** weekly credit is a *derived* `COUNT(session_bookings WHERE status IN ('booked','completed'))`, and `cancel-session` already flips the booking to `cancelled_by_*`, which drops it from the count -- the credit is restored for free, there is no stored counter to reverse. And `calculate_subscription_payout` is a flat monthly amount with no per-session line, so a session refund is a no-op for earnings. The only real gap was server↔FE count divergence: `ClientSessions.tsx` did an optimistic `weeklyUsed--` using a browser-local week boundary that disagrees with the server's Kuwait week. **Fix:** `cancel-session` now returns `{ weeklyUsed, weeklyRemaining, weekStartUtc, weekEndUtc }` computed via the same `get_current_week_bounds()` bounds `book_session_atomic` uses; `ClientSessions.tsx` consumes `result.weeklyUsed` (falling back to the decrement only if the bounds RPC fails). No new RPC, no `refunded` status (the enum has none -- confirmed: `booked, completed, cancelled_by_client, cancelled_by_coach, no_show`). Verified the count semantic live against the real bounds RPC: before-cancel 2 → after-cancel 1 → double-cancel 1 (idempotent). **Residual (B6-N15, deferred):** the *initial-load* weekly count fetch in `ClientSessions.tsx` still uses a browser-local Monday boundary; switching it to `get_current_week_bounds()` is the remaining B6-N15 work.
