...
3. **B7-N2 + B7-N4 + B7-N5 + B7-N6** -- joint migration: `join_team` SECURITY DEFINER RPC (locks capacity, syncs coach_id, enforces cycle gap) + `enforce_subscription_column_whitelist` BEFORE UPDATE trigger + FE swap in ChooseTeamPrompt/ChangeTeamDialog to call the RPC. Backfill query for the prod orphan row included. **Largest single shipment; gate on browser smoke test before publish.**
